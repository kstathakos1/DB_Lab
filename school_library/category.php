...
$conn = getDb();
$book = $conn->query("SELECT *
FROM copies_per_school cps
inner join book_category bc on cps.ISBN = bc.ISBN
inner join category c on bc.category_id = c.category_id
WHERE c.category='$category' AND cps.school_id=$school_id;");
?>
...
<h4 style="margin-top: 5%; margin-left: 10%;">Books in Category <?= $category?> </h4>
<?php if(empty($book)): ?>
